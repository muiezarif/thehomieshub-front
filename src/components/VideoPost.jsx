...
import { ShieldAlert, Lock, Eye, Play, Pause, Volume2, VolumeX } from 'lucide-react';
...
const muxPoster = isMux ? `https://image.mux.com/${playbackId}/thumbnail.jpg?time=1` : null;
  ...
  const thumbnail = post.thumbnail || imageSrcs[post.id % imageSrcs.length];
...
<motion.div
  ...
  onClick={(e) => {
    if (isBlurred) return; // don’t go to watch if locked
    handleOpenWatch(post);
  }}
...
>
        ...
        >
          ...
          <div className="relative aspect-[9/16] w-full overflow-hidden rounded-lg bg-black shadow-md">
            ...
            {/* Video Layer using video element with object-contain */}
  ...
  <MuxPlayer
    ...
    loop
    playsInline
    autoPlay={false}     // you control play via your custom button
    controls={false}     // keep your custom controls
  />
) : (
  <video
    ref={videoRef}
  ...
  />
            ...
            {!isBlurred && (
                <div className={cn(
                    ...
                    {/* Centered Play Button (only when hovering or paused) */}
          ...
          <div className="mt-3 flex items-start space-x-3">
            <Avatar className="h-9 w-9">
              <AvatarImage src={`https://avatar.vercel.sh/${post.creator.name.replace(' ', '')}.png`} alt={post.creator.name} />
              <AvatarFallback>{post.creator.avatar}</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-sm leading-tight text-foreground line-clamp-2">{post.title}</h3>
              <p className="text-xs text-muted-foreground mt-0.5">{post.creator.name}</p>
