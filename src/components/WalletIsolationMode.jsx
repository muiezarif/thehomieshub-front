...
import { 
  X, Minimize2, Maximize2, Wallet, CreditCard, History, 
  Settings, Copy, ExternalLink, ArrowUpRight, ArrowDownLeft, 
  Diamond, Send, ShieldCheck, RefreshCw, ChevronLeft, Image as ImageIcon, Power, Lock 
} from 'lucide-react';
      ...
      <Card className="bg-zinc-900/50 border-primary/20 shadow-glow-gold-sm">
        ...
        <CardContent>
          <div className="flex items-end gap-2">
            <h2 className="text-5xl font-bold text-primary tracking-tight">{balance.toLocaleString()}</h2>
          ...
          {connectedWallet ? (
            <div className="mt-6 p-3 bg-black/40 rounded-lg border border-white/10 flex items-center justify-between">
                ...
                <div className="h-8 w-8 rounded-full bg-white flex items-center justify-center overflow-hidden">
                     {/* Placeholder for wallet icon based on type */}
  ...
  const PurchaseTab = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {[
        ...
        { points: 550, price: 4.99, recommended: true },
        { points: 1200, price: 9.99 },
        { points: 6500, price: 49.99 }
      ].map((pkg) => (
        <div 
          key={pkg.points} 
          className={cn(
          ...
          onClick={() => { addPoints(pkg.points); toast({ title: "Purchase Successful", description: `Added ${pkg.points} pts!`}); }}
        >
          ...
          <Button className={cn("w-full mt-2 font-bold", pkg.recommended ? "bg-primary text-black hover:bg-primary/90" : "bg-white/10 text-white hover:bg-white/20")}>
            ${pkg.price}
          </Button>
        </div>
      ))}
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="grid grid-cols-2 gap-4">
            <Card className="bg-zinc-900 border-primary/20">
                ...
                <CardContent>
                    <div className="text-4xl font-bold text-white">${(balance / 100).toFixed(2)} <span className="text-sm font-normal text-muted-foreground">USD</span></div>
                    <div className="text-xs text-primary mt-1">{balance.toLocaleString()} Points equivalent</div>
                </CardContent>
            </Card>
  ...
  const TransactionsTab = () => (
    <div className="max-w-3xl mx-auto space-y-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
         {MOCK_TRANSACTIONS.map((tx) => (
             <div key={tx.id} className="flex items-center justify-between p-4 rounded-lg bg-zinc-900 border border-white/5 hover:border-primary/20 transition-colors">
                <div className="flex items-center gap-4">
                    <div className={cn("h-10 w-10 rounded-full flex items-center justify-center", tx.type === 'in' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500')}>
                ...
                <div className={cn("font-bold text-lg", tx.amount > 0 ? "text-green-400" : "text-white")}>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black text-white flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        {/* Side Nav for Desktop / Top Scroll for Mobile */}
