...
const FeatureLockedModal = ({ isOpen, onOpenChange }) => {
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      updateUserTier(selectedPlan);
      toast({
        title: '🎉 Upgrade Successful!',
        description: `You are now a ${plans[selectedPlan].name} member.`,
      });
      onOpenChange(false);
    }, 1000);
  };
               ...
               >
                 ...
                 <div>
                   ...
                   <div className="mt-2 flex items-baseline gap-1">
                     <span className="text-3xl font-bold text-white">${billingCycle === 'monthly' ? plan.price.monthly : Math.round(plan.price.yearly/12)}</span>
                     <span className="text-gray-400 text-sm">/mo</span>
                   </div>
                   {billingCycle === 'yearly' && <p className="text-xs text-green-500 font-medium mt-1">Billed ${plan.price.yearly} yearly</p>}
                 </div>
